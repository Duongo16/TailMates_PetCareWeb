...
import { useManagerStats, useManagerMerchants, usePackages } from "@/lib/hooks"
import { managerAPI, packagesAPI } from "@/lib/api"
              ...
              <div className="space-y-3">
                {statsData?.top_merchants?.map((merchant: any, index: number) => (
                  <div key={merchant._id} className="flex items-center gap-4 p-4 rounded-xl bg-secondary/50">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-card ${
                        index === 0 ? "bg-primary" : index === 1 ? "bg-accent" : "bg-foreground"
                      }`}
                    >
                      {index + 1}
                    </div>
